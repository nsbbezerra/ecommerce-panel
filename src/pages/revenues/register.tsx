...
export default function RegisterRevenues() {
  ...
  const handleSubmit: SubmitHandler<RevenueProps> = async (data, { reset }) => {
    try {
      const scheme = Yup.object().shape({
        title: Yup.string().required("Insira um título para a receita"),
        due_date: Yup.date().required("Insira uma data para a receita"),
        value: Yup.string().required("Insira um valor para a receita"),
      ...
      const response = await api.post(`/revenues/${auth?.id}`, data, {
      ...
      <Form ref={formRef} onSubmit={handleSubmit}>
        <Stack spacing={3}>
          ...
          <FormControl>
            ...
            <TextArea placeholder="Descrição da Receita" name="description" />
          </FormControl>
